---
title: "对象的工作原理"
description: "对象不仅仅是多个值的容器，尽管在大多数与对象的交互中，这确实是主要的上下文。

要全面理解 JavaScript 中对象的机制，并充分利用对象在程序中的使用，我们需要更仔细地观察一些特性。这些特性影响对象在交互时的行为。

这些定义对象底层行为的特性统称为“元对象协议”（Metaobject Protocol, MOP）。MOP 不仅有助于理解对象的行为，还可以覆盖对象的默认行为，以更好地适应我们程序的需求。"
