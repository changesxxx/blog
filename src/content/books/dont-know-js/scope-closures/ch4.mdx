---
title: "全局作用域"
description: "第 3 章多次提到“全局作用域”，但你可能仍在想，程序的最外层作用域在现代 JS 中为什么如此重要。绝大多数工作现在是在函数和模块内部完成的，而不是全局范围内。

仅仅声称“避免使用全局作用域”就足够了吗？

JavaScript 程序的全局作用域是一个丰富的话题，其实用性和细微之处远比你想象的要多。本章将首先探讨全局作用域在今天编写 JavaScript 程序时是如何（仍然）有用和相关的，然后查看在不同的 JavaScript 环境中访问全局作用域的不同方式和方法。
