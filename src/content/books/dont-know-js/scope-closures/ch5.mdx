...
description: "到现在为止，你应该对作用域的嵌套有所了解，从全局作用域到下层——这被称为程序的作用域链。

但仅仅知道一个变量来自哪个作用域只是故事的一部分。如果一个变量声明出现在作用域的第一条语句之后，那么在声明之前对该标识符的任何引用将如何表现？如果你尝试在一个作用域中两次声明同一个变量，会发生什么？

JavaScript 特有的词法作用域在变量何时以及如何存在和可用方面充满了细微差别。"
