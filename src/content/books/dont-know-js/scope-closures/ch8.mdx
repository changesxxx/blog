---
title: "模块模式"
description: "在本章中，我们通过探索所有编程中最重要的代码组织模式之一——模块，来结束本书的主要内容。正如我们将看到的，模块本质上是基于我们已经覆盖的内容：学习词法作用域和闭包所带来的回报。

我们已经从广泛的全局作用域到嵌套块作用域，深入研究了变量生命周期的复杂性，全面审视了词法作用域。然后，我们利用词法作用域理解闭包的全部力量。

花一点时间反思一下你在这段旅程中走了多远；你在更深入了解 JavaScript 方面迈出了重要的一步！

本书的核心主题是，理解和掌握作用域和闭包是正确构建和组织代码的关键，尤其是在决定将信息存储在哪些变量时。

我们在最后一章的目标是欣赏模块如何体现这些主题的重要性，将它们从抽象概念提升为构建程序的具体、实用的改进。"
