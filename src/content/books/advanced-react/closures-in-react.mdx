---
title: "React 中的闭包"
description: "在上一章中，我们学习了有关 Refs 的所有知识：它们是什么、我们为何需要它们、何时使用它们以及何时不使用它们。然而，当涉及到在重新渲染之间保持某些内容时，尤其是在 Refs 中，还有一个额外的话题我们需要讨论：函数。"

name: "closures-in-react"
---

更具体地说，闭包及其存在对我们代码的影响。让我们看看几个非常有趣且相当典型的错误，它们是如何出现的，并在此过程中学习：
...
console.log(value); //不起作用，“value” 是本地的
"something" function
```
...
const something = () => {
  ...
  console.log(value); //不起作用，“value” 是 “inside” 函数的本地函数
};
```
...
```jsx
互动示例和完整代码 const first = something('first');
...
如果你还记得 Memoization 章节中的 useMemo、useCallback 和 React.memo，这段代码应该显得熟悉。确实，我们正是实现了 useCallback 钩子为我们做的事情。每次使用 useCallback 时，我们都会创建一个闭包，而我们传递给它的函数被缓存：
...
每次我们点击按钮时，我们记录“未定义”。我们内部的 onClick 值从未更新。你能告诉我为什么吗？当然，答案是过时闭包。当我们创建 onClick 时，闭包首先与默认状态值（即“未定义”）形成。我们将该闭包传递给我们的记忆组件，连同 title 属性。在比较函数内部，我们只比较 title。它从未改变，它只是一个字符串。比较函数始终返回 true，HeavyComponent 从未更新，因此持有对非常第一个 onClick 闭包的引用，冻结的“未定义”值。
