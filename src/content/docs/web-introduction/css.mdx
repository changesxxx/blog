...
CSS 不是真正的编程语言,甚至也不是标记语言;它只是一门样式表语言。其基本目标是让浏览器以指定的特性去绘制页面元素，比如颜色、定位、装饰。
...
**特点**
[官方文档](https://www.w3.org/TR/CSS2/visuren.html#block-formatting)
...
**特点**
[官方文档](https://www.w3.org/TR/CSS2/visuren.html#inline-formatting)
...
> 用来指定行内元素（inline）或表格单元格（table-cell）元素的垂直对齐方式。`vertical-align` 只对行内元素、行内块元素和表格单元格元素生效：不能用它垂直对齐块级元素。
...
对于**文字**，`baseline`是指文本行内字符的底部对齐线。例:x 的下方。

对于`inline-block`如果盒内没有文本默认为`margin-bottom`的底部(如果没有`margin-bottom`则为盒子的底部)。如果有文本则为文字底部对齐线。

<a href="https://codepen.io/weqjfeyg-the-bold/pen/oNVKWMq">示例查看</a>

**vertical-align 属性常见值**
...
<img
  src="./assets/687474703a2f2f7777322e73696e61696d672e636e2f6c617267652f61613066626363346777316635646a3231317262336a32306b6a3039717767382e6a7067.jpeg"
  alt="img"
/>
...
下传递:如果块级元素没有底部填充或边框并且块级元素的底部线和父元素底部线重叠,并且父元素的高度为`auto`(父元素没有单独配置过 height,因为 height 默认值即为 auto),这个块级元素的 margin-bottom 值将会传递给父亲

<a href="https://codepen.io/weqjfeyg-the-bold/pen/zYbQwPR">示例查看</a>

**如何解决 margin 上下传递问题**

给父元素设置`padding-top`或`padding-bottom`，防止顶部线或底部线重叠即可;

父元素设置 border;

父元素触发 BFC:例:设置 overflow 为 auto;

**magrin 上下折叠**

当相邻的垂直方向上的 margin 相遇时，它们会合并成一个外边距，合并后的外边距的值为两者中的较大者;

<a href="https://codepen.io/weqjfeyg-the-bold/pen/YzgbryO">示例查看</a>
...
补充:当我们对绝对定位元素使用了`inset: 0;`时，这个元素会被定位在其包含块的边缘，也就是说它会紧贴着包含块的四个边界。如果此时元素的宽度设置为 auto，那么它的宽度会根据包含块的宽度来自动调整，以确保元素在四个方向上都与的边界一致。
...
<a href="https://codepen.io/weqjfeyg-the-bold/pen/GRebJxa">示例查看</a>
...
1. **clear**
   ...
   示例:
   clear 的本质是告诉 CSS 引擎：在当前元素下方，不允许有浮动元素存在，以确保当前元素能够被浮动元素正确包裹。
