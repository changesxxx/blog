...
<html>
  <head>
    ...
    <script defer="defer" src="https://xxx/main.js"></script>
  </head>
...
<html lang="en">
  ...
  <body>
    <div id="root"></div>
    <script src="reactcdn地址"></script>
    <script src="axioscdn地址"></script>
  </body>
</html>
```
...
`hash` 是整个构建相关的唯一标识符，每次构建过程中它都会生成一个新的值。如果项目中的任何文件发生更改，整个项目的构建散列也会改变。这意味着，即使只改变了一个文件，所有的输出文件名中的 hash 值都会改变，非常不利于浏览器缓存,浏览器需重新缓存资源文件。
...
例如:我们在 index.js 文件中引入一个 css 文件
...
`contenthash` 是针对文件内容本身的散列,只有当文件的实际内容改变时，其 hash 值才会改变。
...
module.exports = {
  ...
  optimization: {
    ...
    usedExports: true, //当为生产模式时 默认为true,如果开发环境需要压缩需手动修改为true
